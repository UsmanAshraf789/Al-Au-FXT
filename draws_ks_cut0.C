...

{
   ...
   TString pdfname = "alau"+Ecm+"_ks_invmass_"+cuttag+".pdf";
   ...
   for(Int_t iPt=0; iPt<kPtBin; iPt++){
	for(Int_t iIM=0; iIM<kCentBin; iIM++){
	   ...
	   TString htitle(Form("K^{0}_{S}, Al+Au %s GeV, %s, p_{T} %s GeV/c", Ecm.Data(), CentName[iIM], PtName[iPt]));
   ...
   TString oname = "alau"+Ecm+"_ks_yield_"+cuttag+".txt";
   ...
   TString oname = "alau"+Ecm+"_ks_invmass_peak_sigma_"+cuttag+".txt";
   ofstream oyield(oname);
   for(int iIM=0;iIM<kCentBin;iIM++)
	for(int iPt=0;iPt<kPtBin;iPt++)
