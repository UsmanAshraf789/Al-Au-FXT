...
//the following will be loaded in .rootlogon.C in ACLIC mode
   ...
   TChain * t = NULL;
   if(!expswitch){
	//t = ChainThem("k0sfp_100_114.pico.list","McV0PicoDst",nlist,block);
	t = ChainThem("k0sfp_100_114.pico.list","McV0PicoDst",nlist,block);
   }
   else{
	//t = ChainThem("k0sexp_100_111.pico.list","McV0PicoDst",nlist,block);
	t = ChainThem("k0sexp_100_111.pico.list","McV0PicoDst",nlist,block);
   }
   ...
   ifstream parms(Form("input/alau5_ks_fit_cut0_%s.txt",funcname[fswitch]));
   if(parms){
	cout<<"READING function parameters from data file "<<Form("input/alau5_ks_fit_cut0_%s.txt",funcname[fswitch])<<endl;
